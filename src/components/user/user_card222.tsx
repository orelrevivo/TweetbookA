...
export function UserCard(user: UserCardProps): JSX.Element {
  ...
  return (
    <Link href={`/user/${username}`}>
      <a
        className='accent-tab34 grid grid-cols-[auto,1fr] gap-3 px-4 py-3 h-[30px]'
      >
       <div className='dgdsgsdgsdgsgsgsege'>
       <UserTooltip avatar {...user} modal={modal}>
          <UserAvatar src={photoURL} alt={name} username={username} />
        </UserTooltip>
       </div>
        <div className='dgdsgsdgsdgsgsgsege flex flex-col gap-1 truncate xs:overflow-visible'>
