...
export type TweetProps = Tweet & {
  ...
  profile?: User | null;
  parentTweet?: boolean;
  tweetUserData?: User; // Ensure the tweet user data is properly typed
};
  ...
  const renderTextWithMentionsAndHashtags = (text: string) => {
    ...
    return parts.map((part, index) => {
      if (index % 2 === 1) { // This is a mention
        return (
          <Link key={index} href={`/user/${part}`}>
            <a className='custom-underline text-main-accent'>@{part}</a>
          </Link>
        );
      }
      // Split again for hashtags within the non-mention part
      const hashtagParts = part.split(hashtagRegex);
      return hashtagParts.map((hashtagPart, hashtagIndex) => {
        if (hashtagIndex % 2 === 1) { // This is a hashtag
          return (
            <Link key={hashtagIndex} href={`/Trends/Hashtag/${hashtagPart}`}>
              <a className='custom-underline text-main-accent'>#{hashtagPart}</a>
            </Link>
          );
        }
        return hashtagPart; // Return regular text
      });
    });
  };
  ...
  return (
    ...
    >
      ...
      <Link href={tweetLink} scroll={!reply}>
        <a
          className={cn(
            `Postarticlemotion accent-tab hover-card relative flex flex-col 
             gap-y-4 px-4 py-3 outline-none duration-200  border-light-border dark:border-dark-border border-1 xs:border`,
          )}
          draggable={false}
            ...
            <div className='flex min-w-0 flex-col'>
              ...
              {text && (
                <p className='whitespace-pre-line break-words'>
                  {renderTextWithMentionsAndHashtags(text)}
                </p>
              )}
              <div className='mt-1 flex flex-col gap-2'>
                ...
                {!modal && (
                  <TweetStats
                    ...
                    viewCount={0}  // Add a placeholder for viewCount
                  />
                )}
              </div>
            </div>
