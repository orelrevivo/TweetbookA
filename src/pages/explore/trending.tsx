    ...
    <>
      <MainContainer className="InputisMobile2">
        ...
        className='group flex items-center justify-between gap-4 rounded-full
                ...
                style={{ width: '95%', height: '40px', transform: 'translateX(13px)', marginTop: '10px' }}
                  ...
                  <div className="mt-4">
                    ...
                    <ul className="list-none p-0">
                      {userResults.map((user) => (
                        <li
                          key={user.uid}
                          className="cursor-pointer accent-tab hover-card p-4"
                          onClick={() => handleUserSelect(user)}
                        >
                          <p className="font-semibold">{user.username}</p>
                          <p className="text-sm text-gray-500 dark:text-gray-300">{user.fullName}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
